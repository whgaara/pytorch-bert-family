        ...
        with open(TrainPath, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if line:
                    ...
                    input_tokens_label, input_tokens_label_id, input_tokens, input_tokens_id = items
                    if not input_tokens:
                        continue
                    input_tokens_id = [int(x) for x in input_tokens_id.split(' ')]
                    input_tokens_label_id = int(input_tokens_label_id)
                    batch_group.append(
                        [input_tokens, input_tokens_id, input_tokens_label, input_tokens_label_id])
